...
const RoomSelection = ({ playerData, onRoomSelect, 
  ...
  const [professionType, setProfessionType] = useState('shared'); // 'individual' или 'shared'
  ...
  useEffect(() => {
    console.log('🔌 [RoomSelection] Component mounted, connecting to socket...');
    ...
    // Ждем подключения перед подпиской на события
    const handleConnect = () => {
    ...
    // Если уже подключены, сразу запрашиваем
    if (socket.connected) {
      handleConnect();
    } else {
